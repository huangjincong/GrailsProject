package com.grails.app.tags

class AplicationTagLib {
    static namespace = "evan"
	
	def resource = { attrs ->
		def dir = attrs.remove("dir")
		def file = attrs.remove("file")
		def absolute = Boolean.valueOf(grailsApplication.config.evan.resource.path.absolute)
		def resourcePath =  grailsApplication.config.evan.resource.contextPath
		def resourceServerURL = grailsApplication.config.evan.resource.serverURL
		if (absolute) {
	...
	def javascript = { attrs ->
		def src = attrs.remove("src")
		out << "<script type=\"text/javascript\" src=\"${resource(dir:"js", file:src)}\" ${paramsToHtmlAttr(attrs)} />"
	}
}
